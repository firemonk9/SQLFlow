...
class MaskTest extends FunSuite with SharedSparkContext  {
  ...
  val testName1 = List(Person("John",123456,"abc Cary, nc"),Person("Rob",123457,"abc Cary, nc"),Person("Mary",123458,"abc Cary, nc"))
  val testName2 = List(Person("John",123456,"abc Cary, nc"),Person("Harry",123434,"abc Cary, nc"), Person("Kerry", 87639, "abc Cary, nc"))
  val testName3 = List(Person("John",123456,"abc Cary, nc"),Person("     ",123454,"abc Carry, nc"), Person(null, 98765, "abc Cary, nc") , Person("TBD", 456789, "abc Cary, nc") , Person("NA", 25467, "abc Cary, nc"))
  ...
  test("testMaskNumber") {
    ...
    val edf = Mask.mask(sampleF,"ssn","SSN",Mask.REAL_NUMBER,true,9,true)
    edf.show()

    val ndf = Mask.mask(sampleFNew,"ssn","SSN",Mask.REAL_NUMBER,true,9,true)
    ndf.show()
  ...
  test("testMaskString SSN") {
    ...
    val tbl1 = Mask.maskColumnVal(test1, "ssn", "ssnStr", true, 9, true)
    tbl1.show()
    val tbl2 = Mask.maskColumnVal(test2, "ssn", "ssnStr", true, 9, true)
    //val tbl2Cnt = tbl2.count()
    tbl2.show()
    val tbl2Head = tbl2.filter(tbl2("name") === "Mike").head()


    val tbl3 = Mask.maskColumnVal(test3, "ssn", "ssnStr", true, 9, true)
    tbl3.show()
    //assert(tbl3.count() == 2)

    val tbl4 = Mask.maskColumnVal(test4, "ssn", "ssnStr", true, 9, true)
    tbl4.show()
...
/*
  ...
  test("testMaskString Name") {
    ...
    */
    val tbl3 = Mask.maskColumnVal(test3, "name", "nameStr", true, 9, true)
